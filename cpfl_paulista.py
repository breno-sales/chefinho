...
def executar(uc: str, cpf_cnpj: str):
    ...
    time.sleep(2)
    pyautogui.click(1443,665) # clicar e preencher cpf/cnpj da uc
    pyautogui.write(cpf_cnpj)
    pyautogui.click(1458,724) # clicar em salvar
    time.sleep(6)
...
def get_dados(n_pages,session=None): # retorna os htmls de cada pagina
    ...
    for page in range(0,n_pages):
        url = f'https://app.solarz.com.br/integrador/faturas/list?page={page}&query={data_pesquisa}&faturaStatusFilter=notInformed&concessionariaFilter=27'
...
def get_csv():
    ...
    conteudo = pandas.read_excel('C:/t1/lista_ucs_ambar.xlsx')
        ...
        for uc_in_page in data_solarz:
            for uc in uc_in_page:
                if str(uc) == str(proposta[0]):
                    ...
                    encontrado = True
                    print('executando...')
                    executar(str(uc),str(proposta[2]))
                    print('executado')
